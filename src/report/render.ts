...
import { describeIsolation, describeRegistryProxy } from "./isolation.js";
...
export function renderText(report: Report): string {
  ...
  const footer = [`overall: ${overallLabel(report)}`, ...(report.isolation ? [`isolation: ${describeIsolation(report.isolation)}`] : []), ...(report.registryProxy ? [`registry proxy: ${describeRegistryProxy(report.registryProxy)}`] : [])];
