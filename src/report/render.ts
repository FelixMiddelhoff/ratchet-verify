...
import { describeIsolation, describeRegistryProxy, describeSuspicious } from "./isolation.js";
...
export function renderText(report: Report): string {
  ...
  const footer = [`overall: ${overallLabel(report)}`, ...(report.isolation ? [`isolation: ${describeIsolation(report.isolation)}`] : []), ...(report.registryProxy ? [`registry proxy: ${describeRegistryProxy(report.registryProxy)}`] : []), ...(report.registryProxy && describeSuspicious(report.registryProxy) ? [describeSuspicious(report.registryProxy)!] : [])];
